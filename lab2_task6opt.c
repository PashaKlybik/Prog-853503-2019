...
{
	int stop = 0, d = 0;
	int dx[4] = {1, 0, -1, 0}; // создаем вспомогательные массивы для расстановки шагов
	int dy[4] = {0, 1, 0, -1};
	matrix[x1][y1] = 0;
...
int main(int argc, const char * argv[]) {
	...
	matrica(x1, y1, x2, y2, firstmatrica, matrix_size);// вызов функции для определения количества шагов
	printf("first number = %d\n", firstmatrica[x2][y2]);
	int min_kolvo = firstmatrica[x2][y2];
	for (int i = 0; i < matrix_size; i++)
		free(firstmatrica[i]);
	free(firstmatrica);
	for (int i = 0; i < matrix_size;i++) // определение наименьшего количества шагов, если убрать одну фигуру
	{
		for (int j = 0; j < matrix_size; j++) {
			if (matrix[i][j] == 1) {  // находим фигуру
				...
				if (secondmatrica[x2][y2] < min_kolvo)
					min_kolvo = secondmatrica[x2][y2];// если получается меньшее значение, присваиваем переменной новое значение
				for (int i = 0; i < matrix_size; i++)
