/*
14. Шар. Разработать программу, меню которой позволяет выполнить следующие функции:
• Ввод радиуса шара
• Вывод площади поверхности шара.
• Вывод объема шара.
• Вывод длины ребра вписанного правильного тетраэдра.
• Вывод площади сечения, проведенного на расстоянии k отцентра шара.
...
int main() {
    ...
    while (1) {
        ...
        do {
            ...
            if (ch == '\n') { continue; }   //ïðîïóñê '\n', ÷òîáû íå âûëåòàäà îøèáêà î íåêîððåêòíîì ââîäå
...
int GetRadius(const int type) {  //ïðèíèìàåò ïåðåìåííóþ èíò, îò êîòîðîé çàâèñèò ñîîáùåíèå, êîòîðîå áóäåò âûâåäåíî
    char str[13];
