...
RubikGLWidget::RubikGLWidget(State *_state)     //окно для вывода 3D модели кубика
{  
    state = _state;
    renderMode = mode_x;
    rotateDelta = 0;   
...
RubikGLWidget::~RubikGLWidget()   //деструктор
{  
    close();                      //закрытие формы
}  

void RubikGLWidget::initializeGL()     //глобальные настройки построения изображения
{  
...
void GLUPerspective(GLfloat fovY, GLfloat aspect, GLfloat zNear, GLfloat zFar )//построение 2D изображения
{
...
void RubikGLWidget::resizeGL(int w, int h) //служит для построения размера окна
{  
...
void RubikGLWidget::paintGL()         // этот метод будет выстраивать каждый кадр для отображения
{  
...
//служит для отображения развертки кубика
...
void RubikGLWidget::drawCell(int face, int i, int j)//рисуем ячейки -границы кубика-Рубика
{
    GLfloat strike = 0.05;
    GLfloat w = (2 - strike * 4) / 3;  
    GLfloat x = 0, y = 0;
...
void RubikGLWidget::keyPressEvent(QKeyEvent *event)  //нажатие на клавиши
{  
    int delay = 200;
    switch (event->key())   
    {   
        case Qt::Key_F1:
            ...
            if (fullscreen)  
            {  
                showFullScreen();      //если нажата F1, то увеличивем размеры кубика
            }  
            else  
            {  
                showNormal();  
            ...
            break; 
        case Qt::Key_Left:
            performRotateView(0, 15, 0, delay); //смещение кубика
            break;
        case Qt::Key_Right:
...
{
    lastPos = event->pos();     //нажатие клавиши
}

void RubikGLWidget::mouseMoveEvent(QMouseEvent *event)  //вращение через нажатие клавиши
{
...
void RubikGLWidget::setColor(int color)      //установка цвета
{
...
void RubikGLWidget::plane_Rotate(string r, int delay)    //изменение характеристик ячеек развертки при вращении
{
    int D = 0;
    rotateLayer = -1;
...
void RubikGLWidget::performRotateView(GLfloat x, GLfloat y, GLfloat z, int delay) //трехмерный поворот с определенной скоростью
{
    QElapsedTimer *timer = new QElapsedTimer();
    timer->start();
